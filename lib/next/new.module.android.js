...
function startBeacons(regionId, beaconUuid, minor, major, message): void {
  BeaconsManager.startBeacons(
    regionId,
    beaconUuid,
    minor ? minor : -1,
    major ? major : -1,
...
function cancelAllNotification(): void {
  BeaconsManager.cancelAllNotification();
}
...
function createNotification(title: string, message: string): void {
  BeaconsManager.createNotification(title, message);
}
